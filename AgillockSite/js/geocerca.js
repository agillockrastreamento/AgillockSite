...
(function () {
  ...
  function init() {
    // auth já foi feita no inline script do HTML
    bindEvents();
    carregar();
  }
  ...
  async function carregar() {
    try {
      geocercas = await AL.apiGet('/api/rastreamento/geocercas');
      atualizarStats();
      renderizar();
    } catch (e) {
  ...
  function atualizarStats() {
    ...
    document.getElementById('stat-visiveis').textContent = geocercas.filter(function (g) { return g.visivelCliente; }).length;
  }
  ...
  function renderizar() {
    ...
    if (!lista.length) {
      tbody.innerHTML =
        ...
        (q ? 'Nenhum resultado para "' + esc(q) + '"' : 'Nenhuma geocerca cadastrada.<br><a href="geocerca-form.html" style="color:#fab32c;font-weight:600;">Criar primeira geocerca</a>') +
        '</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = lista.map(function (g) {
      ...
      var tipoLabel = tipoPair[0];
      var tipoCls   = tipoPair[1];
      var qtd = g.dispositivos ? g.dispositivos.length : 0;
      var dispLabel = qtd === 0
        ? '<span style="color:#aaa">—</span>'
        : qtd + ' dispositivo' + (qtd > 1 ? 's' : '');
      var visivelHtml = g.visivelCliente
        ? '<i class="fa fa-eye" style="color:#27ae60"></i> Sim'
        : '<i class="fa fa-eye-slash" style="color:#bbb"></i> <span style="color:#aaa">Não</span>';
      return '<tr>' +
        '<td><strong>' + esc(g.nome) + '</strong></td>' +
        '<td><span class="geo-badge ' + tipoCls + '">' + tipoLabel + '</span></td>' +
        '<td>' + dispLabel + '</td>' +
        '<td>' + visivelHtml + '</td>' +
        '<td><span class="status-badge ' + (g.ativa ? 'status-ativo' : 'status-inativo') + '">' + (g.ativa ? 'Ativa' : 'Suspensa') + '</span></td>' +
        '<td style="white-space:nowrap">' +
          '<button class="btn-acao" onclick="location.href=\'geocerca-form.html?id=' + esc(g.id) + '\'">' +
    ...
    tbody.querySelectorAll('.btn-suspender').forEach(function (btn) {
      btn.addEventListener('click', function () {
        suspender(btn.dataset.id, btn.dataset.ativa === 'true');
      });
    });
    tbody.querySelectorAll('.btn-excluir').forEach(function (btn) {
      btn.addEventListener('click', function () {
        excluir(btn.dataset.id, btn.dataset.nome);
      });
    });
  }
  ...
  async function suspender(id, ativa) {
    ...
    if (!geo) return;
    try {
      await AL.apiPut('/api/rastreamento/geocercas/' + id, { ativa: !ativa });
      geo.ativa = !ativa;
      atualizarStats();
      renderizar();
      AL.showAlert((geo.ativa ? 'Geocerca ativada.' : 'Geocerca suspensa.'), 'success');
    } catch (e) {
      AL.showAlert('Erro: ' + e.message, 'danger');
    }
  }
  ...
  async function excluir(id, nome) {
    ...
    try {
      await AL.apiDelete('/api/rastreamento/geocercas/' + id);
