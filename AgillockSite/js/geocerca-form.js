...
(function () {
  ...
  function init() {
    ...
    document.title = (editandoId ? 'Editar Geocerca' : 'Nova Geocerca') + ' — AgilLock Admin';

    Promise.all([
      carregarDispositivos(),
      editandoId ? carregarGeocerca(editandoId) : Promise.resolve()
    ]).catch(function (e) {
      AL.showAlert('Erro ao carregar dados: ' + e.message, 'danger');
    });
  ...
  // ── Carregar dispositivos (cache) ──────────────────────────────────────────
  function carregarDispositivos() {
    return AL.apiGet('/api/rastreamento/posicoes').then(function (data) {
  ...
  // ── Carregar geocerca para edição ──────────────────────────────────────────
  function carregarGeocerca(id) {
    return AL.apiGet('/api/rastreamento/geocercas/' + id).then(function (geo) {
      if (!geo) { AL.showAlert('Geocerca não encontrada.', 'danger'); return; }
  ...
  function trocarTile(nome) {
    ...
    if (nome === 'google-mapa') {
      tileGoogle.addTo(mapa); tileAtual = tileGoogle;
    } else if (nome === 'google-sat') {
      tileSat.addTo(mapa); tileAtual = tileSat;
    } else if (nome === 'carto') {
      tileCarto.addTo(mapa); tileAtual = tileCarto;
    }
  }
  ...
  function onMapClick(e) {
    ...
    // Círculo: colocar centro imediatamente
    if (modoDesenho === 'circulo') {
      circuloCenter = { lat: lat, lng: lng };
      var raio = parseInt(document.getElementById('geo-circle-raio').value) || 500;
      limparCamadas();
      camadaDesenho = L.circle([lat, lng], {
        radius: raio, color: '#fab32c', fillColor: '#fab32c', fillOpacity: 0.18, weight: 2
      }).addTo(mapa);
      mapa.panTo([lat, lng]);
      document.getElementById('geo-draw-hint').textContent =
        'Centro definido. Ajuste o raio e clique em Aplicar, ou clique para mover o centro.';
      finalizarCirculo(raio);
      return;
    }

    // Anti-duplo-clique: ignorar o segundo click de um dblclick
    var agora = Date.now();
  ...
  function onMapMouseMove(e) {
    ...
    guiaDesenho = L.polyline(guia, {
      color: '#fab32c', dashArray: '7 6', weight: 2, opacity: 0.7
    }).addTo(mapa);
  }
  ...
  function atualizarDesenhoPoligono() {
    ...
    if (pts.length === 1) {
      camadaDesenho = L.circleMarker(pts[0], {
        radius: 5, color: '#fab32c', fillColor: '#fab32c', fillOpacity: 1, weight: 2
      }).addTo(mapa);
    } else {
      camadaDesenho = L.polygon(pts, {
        color: '#fab32c', fillColor: '#fab32c', fillOpacity: 0.12, weight: 2
      }).addTo(mapa);
    }
  }
  ...
  function finalizarPoligono() {
    ...
    var pts = pontosDesenho.map(function (p) { return [p.lat, p.lng]; });
    camadaDesenho = L.polygon(pts, {
      color: '#27ae60', fillColor: '#27ae60', fillOpacity: 0.2, weight: 2
    }).addTo(mapa);
    mapa.fitBounds(camadaDesenho.getBounds(), { padding: [24, 24] });

    // WKT POLYGON: (lon lat, ...) — fecha com ponto inicial repetido
  ...
  function aplicarRaioCirculo() {
    if (!circuloCenter) {
      AL.showAlert('Clique no mapa primeiro para definir o centro do círculo.', 'warning');
      return;
    }
    var raio = parseInt(document.getElementById('geo-circle-raio').value) || 500;
    limparCamadas();
    ...
    finalizarCirculo(raio);
    document.getElementById('geo-draw-hint').textContent =
      'Círculo atualizado (raio: ' + raio + ' m). Clique no mapa para mover o centro.';
  }
  ...
  function desenharAreaExistente(area, tipo) {
    ...
    try {
      if (tipo === 'circulo' || /^CIRCLE/i.test(area)) {
        ...
        camadaDesenho = L.circle([clat, clng], {
          radius: craio, color: '#27ae60', fillColor: '#27ae60', fillOpacity: 0.2, weight: 2
        }).addTo(mapa);
        ...
        // Remover ponto de fechamento duplicado
        if (pts.length > 1) {
  ...
  function salvar() {
    ...
    if (!nome) {
      AL.showAlert('O nome da geocerca é obrigatório.', 'warning');
      document.getElementById('geo-nome').focus();
      return;
    }
    if (!areaWkt) {
      AL.showAlert('Desenhe uma área no mapa antes de salvar.', 'warning');
      document.getElementById('geo-mapa').scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }
    ...
    req.then(function () {
      AL.showAlert(editandoId ? 'Geocerca atualizada com sucesso!' : 'Geocerca criada com sucesso!', 'success');
      setTimeout(function () {
        window.location.href = 'geocerca.html';
      }, 900);
    }).catch(function (err) {
