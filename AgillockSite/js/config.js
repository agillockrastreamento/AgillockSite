...
window.AL_ICONS_3D = {
  ...
  mapCategoria: function(c) {
    ...
    // Se existir exatamente na lista de shapes, retorna direto
    if (this.shapes[cleanC]) return cleanC;
    
    // Fallbacks de segurança caso o backend envie algo diferente
    ...
    if (/pickup|caminhonete/i.test(cleanC)) return 'carro_assistencia'; // Usa base pickup
  ...
  shapes: {
    ...
    ambulancia: (cor, grad) => window.AL_ICONS_3D.shapes.van('#ecf0f1', 'url(#grad-body-ecf0f1)') + `
      <rect x="-4" y="5" width="8" height="20" fill="#e74c3c" /> <rect x="-10" y="11" width="20" height="8" fill="#e74c3c" /> <rect x="-10" y="-20" width="6" height="3" fill="red" />
      <rect x="4" y="-20" width="6" height="3" fill="blue" />
    `,

    // ==========================================
    // 3. MOTOS E BICICLETAS
    // ==========================================
    ...
    bicicleta: (cor) => `
      <rect x="-1.5" y="-30" width="3" height="60" fill="#333" /> <rect x="-12" y="-20" width="24" height="2.5" rx="1" fill="#7f8c8d" /> <path d="M0,-18 L-3,5 L0,10 L3,5 Z" stroke="${cor}" stroke-width="2" fill="none" /> <ellipse cx="0" cy="5" rx="4" ry="8" fill="#111" /> `,

    // ==========================================
    // 4. CAMINHÕES E TRATORES (BASES)
    // ==========================================
    caminhao: (cor, grad) => `
    ...
    // ==========================================
    // 5. CAMINHÕES COM CARROCERIA FIXA
    // ==========================================
    ...
    caminhao_tanque_combustivel: (cor, grad) => window.AL_ICONS_3D.shapes.caminhao(cor, grad) + `
      <rect x="-18" y="-12" width="36" height="52" rx="18" fill="url(#grad-metal)" /> <circle cx="0" cy="0" r="4" fill="#333" /> <circle cx="0" cy="15" r="4" fill="#333" />
    ...
    caminhao_bomba_concreto: (cor, grad) => window.AL_ICONS_3D.shapes.caminhao(cor, grad) + `
      <rect x="-12" y="-12" width="24" height="50" fill="#95a5a6" /> <path d="M0,0 L15,-15 L10,-35" fill="none" stroke="#e67e22" stroke-width="4" stroke-linejoin="round" /> `,
    ...
    caminhao_recuperacao: (cor, grad) => window.AL_ICONS_3D.shapes.caminhao(cor, grad) + `
      <rect x="-18" y="-12" width="36" height="30" fill="url(#grad-metal)" /> <path d="M-5,10 L0,35 L5,10 Z" fill="#f39c12" /> <line x1="0" y1="35" x2="0" y2="45" stroke="#333" stroke-width="2" /> `,
    ...
    `,
    caminhao_transporte: (cor, grad) => window.AL_ICONS_3D.shapes.caminhao_bau(cor, grad), // Alias para baú longo

    // ==========================================
    // 6. REBOQUES E CONTAINERS (SEM CABINE)
    // ==========================================
    plataforma_reboque: () => `
      <rect x="-20" y="-35" width="40" height="70" fill="url(#grad-metal)" />
      <rect x="-22" y="-5" width="4" height="20" fill="#111" /> <rect x="18" y="-5" width="4" height="20" fill="#111" />
    ...
    escavadeira: (cor, grad) => `
      <rect x="-22" y="-30" width="8" height="60" fill="#222" /> <rect x="14" y="-30" width="8" height="60" fill="#222" /> <rect x="-16" y="-20" width="32" height="40" rx="4" fill="${cor}" /> <rect x="-14" y="-15" width="14" height="20" fill="url(#grad-glass)" /> <path d="M4,-15 L10,-15 L10,-45 L4,-45 Z" fill="#f39c12" /> <path d="M2,-45 L12,-45 L14,-55 L0,-55 Z" fill="#7f8c8d" /> `,
    escavadora: (cor, grad) => window.AL_ICONS_3D.shapes.escavadeira(cor, grad), // Alias 
    ...
    // ==========================================
    // 8. TRANSPORTE DE PASSAGEIROS E AÉREO
    // ==========================================
    onibus: (cor, grad) => `
      <rect x="-18" y="-45" width="36" height="90" rx="4" fill="${cor}" />
      <rect x="-18" y="-45" width="36" height="90" rx="4" fill="${grad}" opacity="0.5" />
      <path d="M-16,-42 L16,-42 L16,-32 L-16,-32 Z" fill="url(#grad-glass)" /> <rect x="-16" y="-25" width="4" height="60" fill="url(#grad-glass)" />
      <rect x="12" y="-25" width="4" height="60" fill="url(#grad-glass)" />
      <rect x="-10" y="-40" width="20" height="4" fill="#333" opacity="0.8" /> `,
    ...
    aviao_passageiros: () => `
      <path d="M-6,-45 Q0,-55 6,-45 L8,35 L0,45 L-8,35 Z" fill="#ecf0f1" /> <path d="M-6,0 L-45,15 L-45,20 L-6,10 Z" fill="#ecf0f1" /> <path d="M6,0 L45,15 L45,20 L6,10 Z" fill="#ecf0f1" /> <path d="M-4,35 L-18,42 L-18,45 L-4,40 Z" fill="#bdc3c7" /> <path d="M4,35 L18,42 L18,45 L4,40 Z" fill="#bdc3c7" /> <rect x="-20" y="12" width="6" height="10" rx="3" fill="#7f8c8d" /> <rect x="14" y="12" width="6" height="10" rx="3" fill="#7f8c8d" /> <path d="M-4,-40 Q0,-45 4,-40 L5,-35 L-5,-35 Z" fill="url(#grad-glass)" /> `,
    drone: () => `
      <circle cx="0" cy="0" r="10" fill="#34495e" /> <circle cx="0" cy="0" r="4" fill="#e74c3c" /> <path d="M-8,-8 L-25,-25 M8,-8 L25,-25 M-8,8 L-25,25 M8,8 L25,25" stroke="#7f8c8d" stroke-width="4" stroke-linecap="round" />
